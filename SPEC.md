...
`sanitize` is the third `icm-kit` command: it projects a private workspace into a shareable form (a remote-support bundle, or a capability harvest). Where §2.5 classifies a file for `audit` and §7 generates the canonical workspace for `init`, §8 classifies a file for **projection**: it assigns every file a **projection home** and the **projection rule** that home carries, and it **fails closed** on anything it cannot home. §8 is normative for that classification foundation. The `sanitize` CLI, the redaction transforms behind `shape_only` and `redact_instance`, the output-tree writer, the manifest, the two modes (`--mode support` / `--mode extract --include`), and the secrets enforcement are additive and staged for later versions; §8 defines only the classification they build on.
